...
> It was generated by `aigis trust-pack` from a fresh `aigis init --agent claude-code --policy enterprise` demo project — nothing here is hand-written.
...
> `aigis init --agent claude-code --policy enterprise` した直後のデモプロジェクトで `aigis trust-pack` を実行して生成したもので、手書きの箇所はありません。
...
- **Generated at / 生成日時:** 2026-06-14T03:51:20.796904+00:00
- **Aigis version / バージョン:** 1.1.11
...
- Signed audit log / 署名付き監査ログ: **available, not yet enabled / 利用可能（未有効化）**
