...
**2. AI事業者ガイドライン v1.2 との対応表を同梱**

経済産業省のAI事業者ガイドライン v1.2 から25項目を抽出し、それぞれに対応する Aigis の機能を対応表にした。「ガイドラインのどのあたりを見ているのか」を1枚で説明できる。

ここは正確に書いておきたい。この対応表の項目ID（`GL-AGENT-01` など）は Aigis が独自に振ったもので、ガイドラインの公式項番ではない。評価も自己評価で、第三者のレビューは受けていない。だから「準拠済みです」と言うためのものではなく、**社内審査に出すときの土台**として使うものだ。審査で「この番号はガイドラインのどこ？」と聞かれたら、公式項番ではないと答える必要がある。

（初出時この節は「37要件すべてに対応」「はい、37/37で答えられる」と書いていた。実際のコード上の項目数は25で、しかも全項目を自分で covered と宣言した数字だった。訂正する。下の画像にも旧表記「37/37 完全準拠」が残っている——画像の差し替え待ちなので、数字は本文を見てほしい。）
